...
class ImageDownloader:
    def __init__(self):
        ...
        self.phantomjs_path = os.path.join(bundle_dir,"phantomjs-2.5.0-beta2-windows","bin","phantomjs.exe")
    
    def get_google_images(self, keyword, path, amount,):
        ...
        crawled_urls = self.crawl_image_urls(keyword, engine="Bing", max_number=amount,
                                             face_only=False, safe_mode=False, browser="phantomjs")
        self.download_images(image_urls=crawled_urls, dst_dir=path, concurrency=4,
                             timeout=20, file_prefix=keyword+"_")
    ...
    def download_image(self, image_url, dst_dir, file_name, timeout=20, proxy_type=None, proxy=None):
        proxies = None
    ...
    def download_images(self, image_urls, dst_dir, file_prefix="img", concurrency=50, timeout=20, proxy_type=None, proxy=None):
        """
        Download image according to given urls and automatically rename them in order.
        :param timeout:
        :param proxy:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
            ...
            for image_url in image_urls:
                ...
                future_list.append(executor.submit(
                    self.download_image, image_url, dst_dir, file_name, timeout, proxy_type, proxy))
                count += 1
    ...
    if sys.platform.startswith("win"):
        phantomjs_path = os.path.join(bundle_dir,"phantomjs-2.5.0-beta2-windows","bin","phantomjs.exe")
    ...
    def my_print(self, msg, quiet=False):
        if not quiet:
            print(msg)
    
    
    def google_gen_query_url(self, keywords, face_only=False, safe_mode=False):
    ...
    def google_image_url_from_webpage(self, driver):
        # time.sleep(10)
        try:
    ...
    def bing_gen_query_url(self, keywords, face_only=False, safe_mode=False):
    ...
    def bing_image_url_from_webpage(self, driver):
        image_urls = list()
    ...
    def baidu_gen_query_url(self, keywords, face_only=False, safe_mode=False):
    ...
    def baidu_image_url_from_webpage(self, driver):
        time.sleep(10)
    ...
    def baidu_get_image_url_using_api(self, keywords, max_number=10000, face_only=False,
    ...
    def crawl_image_urls(self, keywords, engine="Google", max_number=10000,
        ...
        self.my_print("Keywords:  " + keywords, quiet)
        if max_number <= 0:
            self.my_print("Number:  No limit", quiet)
            max_number = 10000
        else:
            self.my_print("Number:  {}".format(max_number), quiet)
        self.my_print("Face Only:  {}".format(str(face_only)), quiet)
        self.my_print("Safe Mode:  {}".format(str(safe_mode)), quiet)
    
        if engine == "Google":
            query_url = self.google_gen_query_url(keywords, face_only, safe_mode)
        elif engine == "Bing":
            query_url = self.bing_gen_query_url(keywords, face_only, safe_mode)
        elif engine == "Baidu":
            query_url = self.baidu_gen_query_url(keywords, face_only, safe_mode)
        else:
            return
    
        self.my_print("Query URL:  " + query_url, quiet)
        ...
        if engine == "Google":
            ...
            image_urls = self.baidu_get_image_url_using_api(keywords, max_number=max_number, face_only=face_only,
